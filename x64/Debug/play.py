...
class WechatAutoJump(object):
    ...
    def get_current_state(self):
        if self.phone == 'Android':
            os.system(path + '\\'+ 'adb shell screencap -p /sdcard/1.png')
            os.system(path +'\\' + 'adb pull /sdcard/1.png ' + path + '\\' + 'state.png')
        elif self.phone == 'IOS':
            self.client.screenshot(path +'\\' + 'state.png')

        if self.debug:
            shutil.copyfile(path + '\\' + 'state.png', os.path.join(self.debug, 'state_{:03d}.png'.format(self.step)))

        state = cv2.imread(path +'\\' + 'state.png')
    ...
    def jump(self, player_pos, target_pos):
        ...
        if self.phone == 'Android':
            ...
            cmd = path +'\\' + 'adb shell input swipe {} {} {} {} {}'.format(press_w, press_h, press_w, press_h, press_time)
            print(cmd)
            os.system(cmd)
        elif self.phone == 'IOS':
...
if __name__ == "__main__":
    print(path)
    AI = WechatAutoJump('Android', 2.045, None, path + '\\'+'resource')
    AI.run()
